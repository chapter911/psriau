...
class AddJabatanMenu extends Migration
{
    public function up()
    {
        ...
        $jabatanId = $this->upsertLv2ByLink('admin/master/jabatan', 'Jabatan', 'fas fa-user-tag', $masterId);

        if ($this->db->tableExists('menu_akses')) {
            $this->ensureAksesForMenu($masterId);
            $this->ensureAksesForMenu($jabatanId);
        }
    }
    ...
    public function down()
    {
        ...
        $menuRow = $this->db->table('menu_lv2')
            ->select('id')
            ->where('LOWER(link)', 'admin/master/jabatan')
            ->get()
            ->getRowArray();
