...
class AddRabGedungSubmenu extends Migration
{
    public function up()
    {
        $db = $this->db;
        ...
        // Find Laporan menu_lv1 id (header)
        $laporanLv1Id = $this->findLv1IdByLabel('laporan');
        if ($laporanLv1Id === null) {
            return;
        }

        $this->ensureLv2Menu($laporanLv1Id, 'RAB Per Gedung', 'admin/laporan/rab-gedung', 'far fa-circle');
    }
    ...
    public function down()
    {
        ...
        $row = $db->table('menu_lv2')
            ->select('id')
            ->where('LOWER(link)', 'admin/laporan/rab-gedung')
            ->get()
            ->getRowArray();
