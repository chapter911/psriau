...
class CreateMasterSimakKonstruksiItem extends Migration
{
    public function up()
    {
        if (! $this->db->tableExists('mst_simak_konstruksi_item')) {
            $this->forge->addField([
                'id' => [
                    'type' => 'INT',
                    'constraint' => 11,
            ...
            $this->forge->createTable('mst_simak_konstruksi_item', true);
        }
    ...
    {
        $this->forge->dropTable('mst_simak_konstruksi_item', true);
    }

    private function seedFromTemplateIfEmpty(): void
    {
        $count = (int) $this->db->table('mst_simak_konstruksi_item')->countAllResults();
        if ($count > 0) {
            return;
        }

        $templateItems = $this->parseLegacyTemplateItems();
        if ($templateItems === []) {
            return;
        }
        ...
        foreach ($templateItems as $item) {
            ...
            $this->db->table('mst_simak_konstruksi_item')->insert($payload);
    ...
    private function parseLegacyTemplateItems(): array
    {
