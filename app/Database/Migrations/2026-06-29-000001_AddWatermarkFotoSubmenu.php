...
class AddWatermarkFotoSubmenu extends Migration
{
    public function up()
    {
        $db = $this->db;
        ...
        // Find Dokumentasi menu_lv1 id (header)
        $dokumentasiLv1Id = $this->findLv1IdByLabel('dokumentasi');
        if ($dokumentasiLv1Id === null) {
            return;
        }

        $this->ensureLv2Menu($dokumentasiLv1Id, 'Watermark Foto', 'admin/dokumentasi/watermark-foto', 'far fa-circle');
    }
    ...
    public function down()
    {
        ...
        $row = $db->table('menu_lv2')
            ->select('id')
            ->where('LOWER(link)', 'admin/dokumentasi/watermark-foto')
            ->get()
            ->getRowArray();
