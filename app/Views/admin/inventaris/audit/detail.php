            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center" style="gap: 8px;">
                ...
                <ul class="nav nav-pills small" style="gap: 4px;">
                    ...
                    <?php foreach ($tabLinks as $key => $label): ?>
                        <li class="nav-item">
                            <a class="nav-link py-1 px-2.5 <?= ($activeTab === $key) ? 'active' : 'text-dark bg-light'; ?>" style="border-radius: 6px;" href="<?= site_url('admin/inventaris/audit/' . $audit['id'] . '?status_audit=' . $key . (! empty($keyword) ? '&q=' . esc($keyword) : '')); ?>">
                                <?= $label; ?>
                            </a>
                        </li>
                    <?php endforeach; ?>
                            ...
                            <?php $no = 1; foreach ($items as $item): ?>
                                <tr id="row-item-<?= $item['id']; ?>" class="item-row <?= ($item['status_audit'] !== 'belum_diperiksa') ? 'table-row-checked' : ''; ?>">
                                    ...
                                    <td class="align-middle small">
                                        ...
                                        <?= esc($item['ruangan_sistem_nama'] ?: ($item['peruntukan'] === 'mobiler' ? 'Sekolah / Mobiler' : 'Belum Ditentukan')); ?>
                                    </td>
