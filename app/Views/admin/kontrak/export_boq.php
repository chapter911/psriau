...
$namaPaket = (string) ($data->nama_paket ?? '');
$namaPenawar = (string) ($data->nama ?? '');
$jabatan = (string) ($data->jabatan ?? 'Konsultan Individual');
$tahunAnggaran = (string) ($data->tahun_anggaran ?? date('Y'));
$durasiPelaksanaan = max(1, (int) ($data->durasi_pelaksanaan ?? 1));
$tanggalSurat = (string) ($data->tanggal_surat_penawaran ?? date('Y-m-d'));
$tanggalCetak = tanggal_indonesia($tanggalSurat !== '' ? $tanggalSurat : date('Y-m-d'));

$nilaiKontrak = (float) ($data->nominal_kontrak ?? 0);
$totalPenawaran = (float) ($data->total_penawaran ?? 0);
if ($totalPenawaran <= 0 && $nilaiKontrak > 0) {
    $totalPenawaran = $nilaiKontrak;
}
if ($totalPenawaran <= 0) {
    $totalPenawaran = 47700000;
}

$personilTotal = (float) round($totalPenawaran * 0.44);
$nonPersonilTotal = (float) ($totalPenawaran - $personilTotal);
$smkkTotal = (float) round($nonPersonilTotal * 0.22);
$sewaTotal = (float) round($nonPersonilTotal * 0.19);
...
$headers = [
    ...
    5 => ['Pekerjaan', ':', $namaPaket !== '' ? $namaPaket : ('Konsultan Individual ' . $jabatan)],
    6 => ['Lokasi', ':', 'Provinsi Riau'],
    7 => ['Tahun Anggaran', ':', $tahunAnggaran],
];
...
// Isi Tabel
$rekapData = [
    ['I.', 'BIAYA LANGSUNG PERSONIL', $personilTotal],
    ['II.', 'BIAYA LANGSUNG NON PERSONIL', $nonPersonilTotal],
];
...
$sheet1->mergeCells('A' . $row . ':B' . $row);
$sheet1->setCellValue('C' . $row, $totalPenawaran);
$sheet1->getStyle('C' . $row)->getNumberFormat()->setFormatCode($currencyFormat);
$sheet1->getStyle('A' . $row . ':C' . $row)->applyFromArray($styleBold);
$row++;
...
// Terbilang
$sheet1->setCellValue('A' . $row, 'Terbilang : ' . ucfirst(terbilang_rupiah((int) $totalPenawaran)));
...
$row = 3;
$sheet2->setCellValue('A'.$row, 'Pekerjaan : ' . ($namaPaket !== '' ? $namaPaket : ('Konsultan Individual ' . $jabatan)));
$row++;
$sheet2->setCellValue('A'.$row, 'Lokasi : Provinsi Riau');
$row++;
$sheet2->setCellValue('A'.$row, 'Tahun Anggaran : ' . $tahunAnggaran);
$row += 2;
...
$sheet2->setCellValue('B'.$row, $jabatan !== '' ? $jabatan : 'Team Leader');
$sheet2->setCellValue('C'.$row, $durasiPelaksanaan);
$sheet2->setCellValue('D'.$row, 'OB');
$sheet2->setCellValue('E'.$row, (float) round($personilTotal / $durasiPelaksanaan));
...
$sheet2->setCellValue('F'.$row, $totalPenawaran);
$sheet2->getStyle('B'.$row)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
...
// Tanda Tangan (HPS)
$row += 2;
$sheet2->setCellValue('E' . $row, 'Pekanbaru, ' . $tanggalCetak);
$row++;
$sheet2->setCellValue('E' . $row, 'Penawar');
$row += 4;
$sheet2->setCellValue('E' . $row, $namaPenawar !== '' ? $namaPenawar : '-');
...
$sheet3->setCellValue('F'.$row, $smkkTotal);
...
$sheet3->setCellValue('F'.$row, $sewaTotal);
$sheet3->getStyle('B'.$row)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
$sheet3->getStyle('A'.$row.':F'.$row)->applyFromArray($styleBold);
$sheet3->getStyle('F'.$row)->getNumberFormat()->setFormatCode($currencyFormat);
$sheet3->getStyle('A'.$startRow.':F'.$row)->applyFromArray($styleBorder);
...
$safePaket = preg_replace('/[^A-Za-z0-9_-]/', '_', strtoupper($namaPaket !== '' ? $namaPaket : 'KONTRAK'));
header('Content-Disposition: attachment;filename="BOQ_' . $tahunAnggaran . '_' . $safePaket . '.xlsx"');
