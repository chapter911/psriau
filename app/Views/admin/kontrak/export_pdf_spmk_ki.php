...
<body>
    ...
    <?php
    $tanggalMulaiRaw = $data->tanggal_awal ?? $data->tanggal_spmk ?? $data->tanggal_kontrak ?? null;
    $tanggalSelesaiRaw = $data->tanggal_akhir ?? null;

    $durasiHari = trim((string) ($data->durasi_pelaksanaan ?? ''));

    if ($durasiHari === '' && ! empty($tanggalMulaiRaw) && ! empty($tanggalSelesaiRaw)) {
        $startTs = strtotime((string) $tanggalMulaiRaw);
        $endTs = strtotime((string) $tanggalSelesaiRaw);
        if ($startTs !== false && $endTs !== false && $endTs >= $startTs) {
            $durasiHari = (string) ((int) floor(($endTs - $startTs) / 86400) + 1);
        }
    }

    if (empty($tanggalSelesaiRaw) && $durasiHari !== '' && ! empty($tanggalMulaiRaw)) {
        $startTs = strtotime((string) $tanggalMulaiRaw);
        if ($startTs !== false && ctype_digit($durasiHari)) {
            $tanggalSelesaiRaw = date('Y-m-d', strtotime('+' . max(((int) $durasiHari) - 1, 0) . ' day', $startTs));
        }
    }

    $tanggalMulaiText = ! empty($tanggalMulaiRaw) ? tanggal_indonesia($tanggalMulaiRaw) : '-';
    $tanggalSelesaiText = ! empty($tanggalSelesaiRaw) ? tanggal_indonesia($tanggalSelesaiRaw) : '-';
    $durasiHariText = $durasiHari !== '' ? $durasiHari : '-';
    ?>
    ...
    <div class="content">
        ...
        <table class="list-table">
            ...
            <tr>
                ...
                <td colspan="3">Waktu Penyelesaian selama (<?= $durasiHariText; ?>) hari kalender dan pekerjaan harus sudah selesai pada tanggal <?= $tanggalSelesaiText; ?>;</td>
            </tr>
            <tr>
