...
class InventarisSmokeTest extends BaseCommand
{
    ...
    public function run(array $params)
    {
        ...
        try {
            ...
            // 4c-2. Uji Nomor Surat Otomatis & Terkunci (Tahun 2026+)
            $testTahun2026 = 2026;
            $autoGenerated2026 = $pinjamModel->generateNextNoSurat($testTahun2026);
            $simulatedRawPost = 'NOMOR-MANUAL-TIDAK-BOLEH';
            $resolvedCreateNoSurat = ($testTahun2026 >= 2026) ? $pinjamModel->generateNextNoSurat($testTahun2026) : $simulatedRawPost;

            $lockTestPinjamId = $pinjamModel->insert([
                'inventaris_id'   => $dummyPinjamAssetId,
                'nama_peminjam'   => 'Pegawai Uji Kunci Nomor',
                'no_surat'        => $resolvedCreateNoSurat,
                'tgl_pinjam'      => '2026-03-01',
                'keperluan'       => 'Uji kunci nomor surat 2026',
                'status'          => 'dikembalikan',
            ]);

            $lockRecord = $pinjamModel->find($lockTestPinjamId);
            $isCreateLocked = ($lockRecord['no_surat'] === $autoGenerated2026);

            $simulatedEditPost = 'COBA-UBAH-NOMOR-2026';
            $existingYear = (int) date('Y', strtotime($lockRecord['tgl_pinjam']));
            $resolvedEditNoSurat = (! empty($lockRecord['no_surat']) && $existingYear === 2026) ? $lockRecord['no_surat'] : $pinjamModel->generateNextNoSurat(2026);

            $pinjamModel->update($lockTestPinjamId, [
                'no_surat'  => $resolvedEditNoSurat,
                'keperluan' => 'Keperluan diperbarui',
            ]);

            $lockRecordAfterEdit = $pinjamModel->find($lockTestPinjamId);
            $isEditLocked = ($lockRecordAfterEdit['no_surat'] === $autoGenerated2026 && $lockRecordAfterEdit['no_surat'] !== $simulatedEditPost);

            $pinjamModel->delete($lockTestPinjamId);

            if ($isCreateLocked && $isEditLocked) {
                CLI::write("  [OK] Proteksi Nomor Surat 2026+ valid: Otomatis terbit ('{$autoGenerated2026}') dan terkunci dari perubahan manual saat create maupun edit", "green");
            } else {
                CLI::error("  [FAIL] Penguncian nomor surat 2026+ gagal.");
            }
